<x-layout :title="$title">
    <section class="p-6 md:p-10">
        <div class="mb-8">
            <h1 class="text-3xl font-bold text-gray-800">Tambah Transaksi Masuk</h1>
            <p class="text-gray-500 mt-1">Isi data berikut untuk menambahkan transaksi baru.</p>
        </div>
        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
            <form method="POST" enctype="multipart/form-data" class="space-y-8" action="{{ route('incoming.store') }}">
                @csrf
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div class="flex flex-col">
                        <label class="font-medium text-gray-700 mb-1">Supplier</label>
                        <select
                            name="supplier_id"
                    ...
                    <div class="flex flex-col">
                        ...
                        <select
                            ...
                            @foreach ($data['product'] as $item)
                                <option value="{{ $item->id }}" data-price="{{ $item->price_buy }}">{{ $item->name }}</option>
                            @endforeach
                        </select>
                    </div>
                    <div class="flex flex-col">
