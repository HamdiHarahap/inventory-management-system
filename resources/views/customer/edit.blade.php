<x-layout :title="$title">
    <section class="p-6 md:p-10">
        <div class="mb-8">
            ...
            <p class="text-gray-500 mt-1">Perbarui data berikut untuk mengubah customer.</p>
        </div>

        <div class="bg-white rounded-2xl shadow-sm border border-gray-200 p-8">
            <form method="POST" class="space-y-8" action="{{ route('customer.update', ['id'=>$data->id]) }}">
                @csrf
                @method('PUT')
                <div class="grid grid-cols-1 gap-6">
                    <div class="flex flex-col">
                        <label class="font-medium text-gray-700 mb-1">Nama Customer</label>
                        <input type="text"
                            name="name"
                            value="{{$data->name}}"
                    ...
                    <div class="flex flex-col">
                        ...
                        <textarea 
                            ...
                            class="px-4 py-3 border border-gray-300 rounded-xl 
                                   focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 
                                   transition shadow-sm"
                            placeholder="Masukkan alamat lengkap supplier">{{$data->address}}</textarea>
                    </div>
                </div>
